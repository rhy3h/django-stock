  ...
  function buildCols(data) {
    ...
    var cols = '<cols>';
    // var parameter = [6, 8, 9, 6, 6, 6, 6, 6, 9, 21, 40]
    for (i=0; i<data.length; i++) {
      colNum = i + 1;
      cols += '<col min="' + colNum + '" max="' + colNum + '" width="10" customWidth="1"/>';
    }
  ...
  function getTableData(table) {
    ...
    table.rows().every( function ( rowIdx, tableLoop, rowLoop ) {
      var data = this.data();
      data[0] = data[0].replace(/<a\b[^>]*>/gm, '').replace(/<\/a>/gm, '');
      data[4] = data[4].replace(/<p\b[^>]*>/gm, '').replace(/<\/p>/gm, '');
